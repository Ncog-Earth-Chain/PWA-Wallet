/**
 * This Implements Ncogearthchain Nano Ledger HW Wallet API
 *
 * @author Jiri Malek <jirka.malek@gmail.com>
 * @copyright (c) 2022, Ncogearthchain
 * @version 0.1.7
 * @licese MIT
 */
...
// NCOGEARTHCHAIN_CHAIN_ID represents the Ncogearthchain Ncogearthchain main chain id.
export const NCOGEARTHCHAIN_CHAIN_ID = 0xa09;

// CLA specified service class used by Ncogearthchain Ledger application
const CLA = 0xe0;

// INS specifies instructions supported by the Ncogearthchain Ledger app
const INS = {
    GET_VERSION: 0x01,
    GET_PUBLIC_KEY: 0x10,
    GET_ADDRESS: 0x11,
...
// ErrorCodes exports list of errors produced by the Ncogearthchain Ledger app
...
// NcogearthchainNano implements high level Ncogearthchain Nano Ledger HW wallet communication
export default class NcogearthchainNano {
    ...
    /**
     * Construct new NcogearthchainNano API bridge
     *
     * @param {Transport} transport
     * @param {string} ledgerAppKey APDU proxy key of the Ledger application
     */
    constructor(transport, ledgerAppKey = "NEC") {
        // keep the transport
        ...
        // in the mean time, the Ncogearthchain app will restart the Ledger device to protect
    ...
    /**
     * getVersion obtains Ncogearthchain Nano Ledger application version
     *
    ...
    /**
     * getAddress extracts Ncogearthchain wallet address for the given account and address id.
     *
    ...
    /**
     * listAddresses extracts sequence of logically consequent Ncogearthchain wallet addresses
    ...
    /**
     * getPublicKey derives Ncogearthchain wallet public key for the given account and address id.
     *
     * @param {number} accountId Zero based account identifier.
     * @param {number} addressId Zero based address identifier.
     * @returns {Promise<{}>}
    ...
    async signTransaction(accountId, addressId, tx) {
        ...
        const txFinal = new Transaction(
            {
                ...tx,
                v: sig.v + ((NCOGEARTHCHAIN_CHAIN_ID * 2) + 8),
                r: sig.r,
                s: sig.s
            },
     ...
     *
     * Please note that the Ncogearthchain Ledger application is coded to provide
     * only subset of BIP32 paths with prefix "44'/60'". We don't derive
     * addresses outside of expected Ncogearthchain address space.
     *
     * @param {[]} bip32Path
     ...
     * Please note that the Ncogearthchain Ledger application is coded to provide
     * only subset of BIP32 paths with prefix "44'/60'". We don't derive
     * public keys outside of expected Ncogearthchain address space.
     *
     * @param bip32Path
     * @returns {Promise<{}>}
     */
