...
import NcogearthchainNano from "../../lib/ncogearthchain-nano";
 ...
 * it into the console
 *
 * @param {NcogearthchainNano} bridge
 * @returns {Promise<void>}
 */
async function showVersion(bridge) {
    // inform
    console.log("\nRequesting Ncogearthchain Nano Ledger app version.");

    // get the version info
    const ver = await bridge.getVersion();
    console.log(`    > Ncogearthchain Nano app version is: ${ver.major}.${ver.minor}.${ver.patch}`);
...
async function getSignedTransaction(bridge) {
    ...
    const tx = {
        ...
        value: "0xde0b6b3a7640000", /* 1 NEC in WEI (1e+18) in HEX */
...
async function run() {
    // inform
    console.log("-----------------------------------------------");
    console.log("Ncogearthchain Nano Ledger application example started.");
    console.log("-----------------------------------------------");
    ...
    // init the API bridge
    const bridge = new NcogearthchainNano(tr);
    try {
