 ...
 */
class NcogearthchainEventEmitter {
 ...
 */
class NcogearthchainInpageProvider extends NcogearthchainEventEmitter {
    ...
    _sendToContentScript(payload, callback = null) {
        ...
        if (this.debug) console.log('NcogearthchainPwaWallet request', id, payload);
        window.postMessage({
            target: 'NcogearthchainPWAwalletBackground',
            data: payload,
            msgId: id,
        }, location.origin);
    }
    ...
    _receiveFromContentScript(event) {
        ...
        if (msg.target !== 'NcogearthchainPWAwalletInpage') return
        if (!msg.data) return
        let data = msg.data;

        if (this.debug) console.log('NcogearthchainPwaWallet response', msg.msgId, data);
        ...
        if (msg.msgId && typeof this.messageCallbacks[msg.msgId] !== 'undefined') {
            ...
            if (data.error) {
                console.log('NcogearthchainPwaWallet RPC error', data);
                callback(data.error, null); // (error, result)
            } else {
                callback(null, data); // (error, result)
            }
            if (this.debug) console.log('NcogearthchainPwaWallet response callback called', msg.msgId, data);
        }
    ...
    _handleAccountsChanged (accounts, isEthAccounts = false, isInternal = false) {
        if (this.debug) console.log('NcogearthchainPwaWallet accountsChanged', accounts);
        this.emit('accountsChanged', accounts)
    }
}

window.ethereum = new NcogearthchainInpageProvider();
...
console.log("Ncogearthchain-PWA-Wallet initialized");
