...
function injectScriptFile(url) {
    try {
        ...
        console.error('Ncogearthchain-PWA-Wallet inpage injection failed.', e);
    }
}
...
function sendToInpage() {
    window.postMessage({
        target: 'NcogearthchainPWAwalletInpage',
        data: data,
    }, location.origin);
}
...
window.addEventListener('message', function(event) {
    ...
    if (msg.target !== 'NcogearthchainPWAwalletBackground') return
    if (!msg.data) return
