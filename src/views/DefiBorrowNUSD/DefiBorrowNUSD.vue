<template>
    <div class="defi-borrow-nusd">
        ...
        <h2 class="perex">
            Mint nusd with your locked {{ wnecTokenSymbol }} or repay nusd to unlock your {{ wnecTokenSymbol }}. You can
            use nusd to trade synths, lend it to the liquidity pool to earn interest, and use it as a collateral to
            borrow synths.
        </h2>
        ...
        <div class="grid">
            <div>
                ...
                <div v-else class="df-data-item smaller">
                    <h3 class="label">nusd balance</h3>
                    <div class="value">{{ debt }} <span class="currency">nusd</span></div>
                </div>
                -->
            ...
            <div class="defi-price-input-col align-center">
                <div class="defi-price-input">
                    ...
                    <div class="token-label">nusd</div>
                    <div class="token-info">
                        <div class="token-info-label">Repay<br />nusd</div>
                        <icon data="@/assets/svg/angle-double-left.svg" width="66" height="66" aria-hidden="true" />
                    </div>
                    <div class="token-info increase">
                        <div class="token-info-label">Mint<br />nusd</div>
            ...
            <div v-if="largeView" class="right-col">
                ...
                <div v-if="smallView" class="df-data-item smaller">
                    ...
                    <div class="value">
                        <f-placeholder :content-loaded="!!nusdToken.symbol" replacement-text="99%">
            ...
            <f-message v-if="increasedDebt > 0" type="info" role="alert" class="big">
                ...
                <span class="inc-desc-collateral">
                    <f-token-value :token="nusdToken" :value="increasedDebt" no-currency /> nusd
                </span>
            </f-message>
            <f-message v-else-if="decreasedDebt > 0" type="info" role="alert" class="big">
                You’re removing
                <span class="inc-desc-collateral">
                    <f-token-value :token="nusdToken" :value="decreasedDebt" no-currency /> nusd
                </span>
            </f-message>
        </div>
...
export default {
    ...
    computed: {
        ...
        debtLimit() {
            const debtNUSD = parseFloat(this.debt);
            const currDebtNUSD = parseFloat(this.currDebt) - debtNUSD;

            return this.$defi.getDebtLimit(this.fMintAccount, currDebtNUSD);
        },
        ...
        backButtonRoute() {
            const parentNode = getAppParentNode('defi-borrow-nusd');
    ...
    methods: {
        ...
        formatInputValue(_value) {
            return parseFloat(_value).toFixed(this.$defi.getTokenDecimals(this.nusdToken));
        },
