    ...
    <div class="defi-ftrade-confirmation">
        ...
        >
            ...
            <template #window-content>
                <ledger-confirmation-content :to="tx.to" :amount="amountNEC" :max-fee="tx._fee" />
            </template>
        </tx-confirmation>
        <template v-else>
...
import wnecUtils from 'ncogearthchain-ledgerjs/src/wnec-utils.js';
...
/**
 * Common component for DefiBorrowNUSDConfirmation a DefiManageBorrowConfirmation
 */
export default {
        ...
        async setTx() {
            ...
            */
            ...
            if (this.params.step === 1) {
                ...
                if (fromToken.symbol === 'NEC' && toToken.canWrapNEC) {
                    this.amountNEC = params.fromValue.toFixed(this.$defi.getTokenDecimals(params.fromToken));

                    txToSign = wnecUtils.defiWrapNec(
                        toToken.address,
                        Web3.utils.toHex(this.$defi.shiftDecPointRight(params.toValue.toString(), toToken.decimals))
                    );
                } else if (fromToken.canWrapNEC && toToken.symbol === 'NEC') {
                    const amount = Web3.utils.toHex(
                        this.$defi.shiftDecPointRight(params.fromValue.toString(), toToken.decimals)
                    );

                    txToSign = wnecUtils.defiUnwrapNec(
                        fromToken.address,
