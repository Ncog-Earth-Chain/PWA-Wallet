...
// Ncogearthchain web3 wallet plugin for VUE, based on https://git`hub.com/Ncogearthchain-io/ncogearthchain-web3-wallet
export class NcogearthchainWeb3Wallet {
    static install(_Vue, _options) {
        fWallet = new NcogearthchainWeb3Wallet(_options);
        ...
        this.blockchains = [
            {
                ...
                label: 'Ncogearthchain Ncogearthchain',
            },
            {
                value: 'ethereum',
                label: 'Ethereum',
    ...
    async getTransactionToSign({ from, to, value, memo = '' }) {
        ...
        return {
            ...
            _fee: !error ? this.WEIToNEC(this.getTransactionFee(gasPrice, gasLimit)) : 0,
        };
    }
    ...
    */
    ...
    /**
     * Get the remaining balance (in NEC) after deducting transaction fee.
     *
     * @param {*} _balance
     * @param {*} _gasPrice
     * @param {string} [_gasLimit]
    ...
    getRemainingBalance(_balance, _gasPrice, _gasLimit) {
        ...
        return parseFloat(this.WEIToNEC(balance.sub(fee.mul(this.toBN(2)))));
    }
    ...
    getMaxRemainingBalance(_balance, _gasPrice) {
        ...
        return parseFloat(this.WEIToNEC(balance.sub(fee.mul(this.toBN(1)))));
    }
    ...
    /**
     * @param _address
     * @param {('ncogearthchain' | 'ethereum' | 'binance')} _blockchain
     * @return {boolean}
     */
    isValidAddress(_address, _blockchain = 'ncogearthchain') {
        if (_blockchain === 'ncogearthchain' || _blockchain === 'ethereum') {
