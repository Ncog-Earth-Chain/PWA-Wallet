...
export class BNBridgeExchange {
    ...
    /**
     * Get ncogearthchain token from tokens list.
     *
     * @return {BNBridgeToken|undefined}
     */
    async getNcogearthchainToken() {
        const tokens = await this.getTokens();

        return tokens ? tokens.find((_token) => _token.symbol === 'NEC') : undefined;
    }
    ...
    async getETHBalance(_ethReceiveAddress, _token) {
        const token = /** @type {BNBridgeToken} */ _token || (await this.getNcogearthchainToken());
        const pData = {
     ...
     * @param {string} [ncogearthchainAddress]
     * @param {BNBridgeDirection} [direction]
     * @param {BNBridgeToken} [_token]
     * @return {Promise<null>}
     */
    async swapToken({ ethAddress = '', bnbAddress = '', ncogearthchainAddress = '', direction = 'EthereumToBinance' }, _token) {
        const token = /** @type {BNBridgeToken} */ _token || (await this.getNcogearthchainToken());
        const pData = {
            direction,
            token_uuid: token ? token.uuid : '',
            ...
            ncogearthchain_address: ncogearthchainAddress,
        };
            ...
            if (data) {
                if (data.success) {
                    ...
                    if (
                        ...
                        pData.ncogearthchain_address !== pData.ncogearthchain_address.toLowerCase()
                    ) {
                        // resend request with Ncogearthchain address in lowercase
                        pData.ncogearthchain_address = pData.ncogearthchain_address.toLowerCase();
     ...
     * @return {string}
     */
    getNECCurrencyByDirection(_direction) {
        let currency = 'NEC';

        if (_direction === 'NcogearthchainToNcogearthchain') {
            currency += '-Ncogearthchain';
