        ...
        <div class="grid">
            <div>
                ...
                <div v-if="!largeView" class="df-data-item smaller">
                    ...
                    <div class="value"><f-token-value :token="nusdToken" :value="overallCollateral" /></div>
                </div>
                <div v-if="!largeView" class="df-data-item smaller">
                    <h3 class="label">
                    ...
                    <div class="value"><f-token-value :token="nusdToken" :value="overallDebt" /></div>
                </div>
            </div>
            ...
            <div v-if="largeView" class="right-col">
                ...
                <div class="df-data-item smaller">
                    ...
                    <div class="value">{{ overallCollateral.toFixed(3) }} <span class="currency">nusd</span></div>
                </div>
                <div class="df-data-item smaller">
                    <h3 class="label">Total Borrowed</h3>
                    <div class="value">{{ overallDebt.toFixed(3) }} <span class="currency">nusd</span></div>
                </div>
                -->
...
*/
        ...
        maxMintable() {
            /*
            const { $defi } = this;
            const nusdToken = this.tokens.find((_item) => _item.symbol === 'NUSD');
            const totalDebtNUSD = $defi.fromTokenValue(this.fMintAccount.debtValue, nusdToken);
            const totalCollateralNUSD = $defi.fromTokenValue(this.fMintAccount.collateralValue, nusdToken);
            const borrowLimitNUSD = $defi.getMaxDebtNUSD(totalCollateralNUSD) - totalDebtNUSD;

            return borrowLimitNUSD / this.tokenPrice;
*/
        ...
        currCollateralNUSD() {
            ...
            const collateralNUSD = collateral * this.tokenPrice;
            ...
            return cCollateral * this.tokenPrice - collateralNUSD;
        },

        debtLimit() {
            return this.$defi.getDebtLimit(this.fMintAccount, 0, this.currCollateralNUSD);
            // return this.$defi.getMintingLimit(this.debt, this.currCollateral, this.tokenPrice);
        },

        collateralRatio() {
            return this.$defi.getCollateralRatio(this.fMintAccount, 0, this.currCollateralNUSD);
        },
    ...
    methods: {
        async init(_dontSetDToken) {
            ...
            if (!this.singleToken) {
                ...
                if (this.lockUnlockMode) {
                    this.tokens = this.tokens.filter((_token) => _token.symbol !== 'NUSD');
                }
            }
        ...
        updateMessage() {
            ...
            if (this.availableNEC <= 0.01) {
                this.message = 'Deposit more NEC to increase your collateral';
            } else {
                this.message = '';
            }
*/
