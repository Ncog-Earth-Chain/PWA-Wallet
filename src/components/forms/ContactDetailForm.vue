...
export default {
    ...
    mounted() {
        ...
        this.blockchain = this.contactData.blockchain || 'ncogearthchain';
    },
    ...
    methods: {
        ...
        async checkAddress(_value) {
            ...
            let address = _value;
            if (blockchain === 'ncogearthchain' && resolution.isSupportedDomainInNetwork(_value)) {
                try {
                    this.resolvedAddress = address = await resolution.multiChainAddr(_value, 'NEC', 'NCOGEARTHCHAIN');
                } catch (e) {
            ...
            if (!this.$fWallet.isValidAddress(address, blockchain)) {
                if (blockchain === 'ncogearthchain') {
                    this.addressErrorMsg = 'Not valid ncogearthchain address';
            ...
            if (
                ...
                (blockchain === 'ncogearthchain' && this.getAccountAndIndexByAddress(address).index !== -1)
            ) {
