                ...
                <fieldset class="">
                    <div class="form-body">
                        ...
                        <address-field
                            ...
                            :validate-on-input="d_sendDirection === 'NcogearthchainToNcogearthchain'"
                        >
                            <template #bottom="sProps">
                                <f-message v-show="sProps.showErrorMessage" type="error" alert with-icon>
                                    {{ sendToErrorMsg }}
                        ...
                        <f-input
                            v-if="d_sendDirection !== 'NcogearthchainToEthereum' && !token.address"
                            label="Memo (optional)"
                            field-size="large"
                            name="memo"
                        />
                        ...
                        <div class="align-center form-buttons">
                            <template v-if="d_sendDirection !== 'NcogearthchainToNcogearthchain'">
                                <f-message type="warning" class="align-center">
                                    All bridge transactions incur a fee of {{ minNECToTransfer }} NEC, deducted from the
                                    transfer amount.
                                </f-message>
                                <f-message
                                    v-if="amount >= minNECToTransfer && checkAmount(amount)"
                                    type="info"
                                    class="big"
                                >
                                    You will receive <b>{{ amount - minNECToTransfer }} NEC</b>
                                </f-message>
                                <br />
                            </template>
...
export default {
    ...
    data() {
        return {
            d_sendDirection: 'NcogearthchainToNcogearthchain',
    ...
    computed: {
        ...
        sendToLabel() {
            ...
            switch (this.d_sendDirection) {
                case 'NcogearthchainToBinance':
                    sendTo = 'BNB Receive Address';
                    break;
                case 'NcogearthchainToEthereum':
    ...
    methods: {
        ...
        async checkAddress(_value) {
            ...
            if (d_sendDirection === 'NcogearthchainToNcogearthchain') {
                value = (await this.resolveAddress(value, 'NEC', 'NCOGEARTHCHAIN')) || value;
                console.log(value);
                validAddress = this.$fWallet.isValidAddress(value);
                this.sendToErrorMsg = 'Enter a valid Ncogearthchain NEC address or domain name';
            } else if (d_sendDirection === 'NcogearthchainToBinance') {
                ...
                if (validAddress) {
                    if (value.toLowerCase() === 'bnb136ns6lfw4zs5hg4n85vdthaad7hq5m4gtkgf23') {
                        ...
                        try {
                            ...
                            this.ETHOrBNBAccountBalance = `Current Ncogearthchain Balance: ${data.balance} NEC`;
                        } catch (_error) {
                            validAddress = false;
            ...
            } else if (d_sendDirection === 'NcogearthchainToEthereum') {
                ...
                if (validAddress) {
                    try {
                        ...
                        this.ETHOrBNBAccountBalance = `Current Ncogearthchain Balance: ${balance} NEC`;
                    } catch (_error) {
                        validAddress = false;
        ...
        async resolveAddress(value, currency, chain) {
            ...
            console.log(resolution.isSupportedDomainInNetwork(value));
        ...
        checkAmount(_value) {
            ...
            const { minNECToTransfer } = this;
            const ncogearthchainToBridge = this.d_sendDirection !== 'NcogearthchainToNcogearthchain';
            ...
            if (!isNaN(value)) {
                if (value <= remainingBalance && value > 0) {
                    if (ncogearthchainToBridge && value < minNECToTransfer) {
                        this.amountErrMsg = `You must transfer at least ${minNECToTransfer} ${tokenSymbol}`;
                    } else {
                        ok = true;
                    }
        ...
        */
        async onFormSubmit(_event) {
            ...
            if (this.currentAccount && data.amount) {
                if (d_sendDirection === 'NcogearthchainToNcogearthchain') {
                    data.ncogearthchain_address = this.resolvedAddress || data.address;
                } else if (d_sendDirection === 'NcogearthchainToBinance') {
                ...
                this.$emit('change-component', {
                    ...
                    data: {
                        ...
                        amount: '0.1',
                        fee: '0.000044',
                        from_ncogearthchain_address: '0x76AE07E6D236c1aE3F5C3112F387ad82c69A2471',
                        memo: '',

                        bnb_address: 'bnb1jvlepaalght59kwhfh44k54u3elmhuurnt0lxc',
                        direction: 'NcogearthchainToBinance',
                        ncogearthchain_address: '0xE504aF2999644A86162Df892E86E3809a365AEBa',
...
*/
                ...
                /*this.$emit('change-component', {
                    ...
                    data: {
                        ...data,
                        // fee: this.$fWallet.WEIToNEC(this.$fWallet.getTransactionFee(this.gasPrice)),
                    },
                });*/
            }
        },
