<template>
    <div class="stake-form" :class="{ 'increase-delegation': d_increaseDelegation }">
        <h1 :id="labelId" data-focus class="with-back-btn align-center" aria-label="Delegate NEC">
            <span>Delegate NEC</span>
                ...
                <fieldset class="">
                    <div class="form-body">
                        ...
                        >
                            <template #top="sProps">
                                <div class="input-label-layout">
                                    ...
                                    <button type="button" class="btn light small c-green" @click="onEntireBalanceClick">
                        ...
                        <div class="align-center form-buttons">
                            <button type="submit" class="btn large break-word custom-continue">
                                Continue
                            </button>
                        </div>
                    </div>
...
import { GAS_LIMITS } from '../../plugins/ncogearthchain-web3-wallet.js';
...
export default {
    ...
    methods: {
        ...
        checkAmount(_value) {
            ...
            if (!isNaN(value)) {
                if (value <= remainingBalance && value >= 1) {
                    ...
                    this.amountErrMsg = `You can't stake amount less than 1 NEC`;
                } else if (value >= 1) {
                    this.amountErrMsg = `You can stake max ${remainingBalance} NEC`;
                }
            }

            if (ok && this.validatorInfo.delegatedLimit && this.validatorInfo.address) {
                const delegatedLimit = parseFloat(this.$fWallet.WEIToNEC(this.validatorInfo.delegatedLimit));

                if (value > delegatedLimit) {
                    this.amountErrMsg =
                        `Staking limit reached. You can stake max ${delegatedLimit} NEC on validator ` + this.validator;
                    ok = false;
                }
            }
         ...
         *
         * @param {Number} _amount Amount of NEC to stake.
         * @return {Promise<void>}
         */
