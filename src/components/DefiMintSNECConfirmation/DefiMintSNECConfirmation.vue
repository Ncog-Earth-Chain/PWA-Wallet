<template>
    <div class="defi-mint-snec-confirmation">
        <tx-confirmation
            v-if="hasCorrectParams"
            :tx="tx"
            send-button-label="Submit"
            password-label="Please enter your wallet password to mint sNEC"
        ...
        >
            <h2 class="not-visible" data-focus>Mint sNEC - Confirmation</h2>
...
export default {
    ...
    methods: {
        async setTx() {
            ...
            this.dAmountDelegated = this.$fWallet.WEIToNEC(this.d_amountDelegated);
        ...
        onSendTransactionSuccess(_data) {
            if (!this.isView) {
                this.$emit('change-component', {
                    to: 'staking-mint-snec-confirmation-success-message',
                    from: this.compName,
                    data: {
                ...
                this.$router.replace({
                    name: `defi-mint-snec-transaction-success-message`,
                    params,
                });
            }
        },
        ...
        onChangeComponent(_data) {
            let transactionRejectComp = `defi-mint-snec-transaction-reject-message`;
