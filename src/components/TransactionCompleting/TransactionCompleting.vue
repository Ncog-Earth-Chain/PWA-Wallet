        ...
        <template v-if="success">
            ...
            <ul class="no-markers" style="margin-bottom: 16px;">
                <li v-for="result in tsData.result" :key="result.uuid">
                    <b>
                        {{ result.amount }}
                        {{ $bnb.getNECCurrencyByDirection(receive ? tsData.direction : 'NcogearthchainToNcogearthchain') }}
                    </b>
                    from
                    <b class="break-word">{{
                        receive ? tsData[$bnb.getAddressKeyByDirection(tsData.direction)] : tsData.from_ncogearthchain_address
                    }}</b>
                </li>
            </ul>
            ...
            <p>
                ...
                <b>
                    ...
                    {{ $bnb.getNECCurrencyByDirection(receive ? 'NcogearthchainToNcogearthchain' : tsData.direction) }}
                </b>
                in your address
                <b> {{ tsData.address || tsData.result[0].ncogearthchain_address }}</b>
            </p>
