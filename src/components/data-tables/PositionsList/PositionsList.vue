<template>
    <div class="open-positions-list-dt">
        ...
        >
            ...
            <template v-slot:column-amount_nusd="{ value, item, column }">
                <div v-if="column" class="row no-collapse no-vert-col-padding">
                    <div class="col-6 f-row-label">{{ column.label }}</div>
                    <div class="col break-word">
                        <template v-if="item._collateral > 0">{{ formatCollateralNUSD(item) }}</template>
                        <template v-if="item._debt > 0">{{ formatDebtNUSD(item) }}</template>
                    </div>
                </div>
                <template v-else>
                    <template v-if="item._collateral > 0">{{ formatCollateralNUSD(item) }}</template>
                    ...
                    <div class="col break-word">
                        <template v-if="canClaimRewards(item.rewards)">
                            <f-token-value
                                ...
                                :token="wnecToken"
                                :value="pendingRewardsWNEC(item.rewards) - stashedRewardsWNEC(item.rewards)"
                            />
                            <span class="currency-light">
                                ...
                                <f-token-value
                                    :use-placeholder="false"
                                    :token="wnecToken"
                                    :value="stashedRewardsWNEC(item.rewards)"
                                />
                            </span>
                        </template>
                ...
                <div v-if="column" class="row no-collapse no-vert-col-padding">
                    ...
                    <div class="col break-word">
                        <template v-if="item._collateral > 0 && usedAsCollateral(item)">
                            ...
                            <template v-if="item.symbol === 'WNEC'">
                                ,
                        ...
                        <template v-if="canClaimRewards(item.rewards)">
                            ,<a
                                ...
                                href="#"
                                :data-pending-rewards="pendingRewardsWNEC(item.rewards)"
...
import { MAX_TOKEN_DECIMALS_IN_TABLES } from '@/plugins/ncogearthchain-web3-wallet.js';
...
export default {
    ...
    watch: {
        ...
        async tokens(_value) {
            let tokens = _value.filter((_item) => {
                return _item.isActive && (_item.canDeposit || _item.canMint) && _item.symbol !== 'NEC';
            });

            this.wnecToken = _value.find((_item) => _item.symbol === 'WNEC');
    ...
    methods: {
         ...
         * @return {*|number}
         */
        formatDebtNUSD(_token) {
            const debt = this.getDebt(_token);

            return debt > 0
                ? formatNumberByLocale(
                      debt * this.defi.getTokenPrice(_token),
                      this.defi.getTokenDecimals({ symbol: 'NUSD' }, MAX_TOKEN_DECIMALS_IN_TABLES)
                  )
                : 0;
        },
        ...
        usedAsCollateral(_token) {
            return _token.symbol === 'WNEC' || _token.symbol === 'SNEC';
        },
         ...
         * @param {object} _rewards
         */
        pendingRewardsWNEC(_rewards) {
            return this.$defi.fromTokenValue(_rewards.rewardsEarned, this.wnecToken) || 0;
        },

        /**
         * @param {object} _rewards
         */
        stashedRewardsWNEC(_rewards) {
            return this.$defi.fromTokenValue(_rewards.rewardsStashed, this.wnecToken) || 0;
        },
