<template>
    <div class="receive-coin">
        <div class="align-center">
            Send your {{ $bnb.getNECCurrencyByDirection(direction) }} to:
...
export default {
    ...
    computed: {
        ...
        direction() {
            if (this.coin === 'BNB') {
                return 'BinanceToNcogearthchain';
            } else {
                return 'EthereumToNcogearthchain';
            }
        },
    },
        ...
        async setAddress() {
            try {
                ...
                this.stData = await this.$bnb.swapToken({
                    direction,
                    ncogearthchainAddress: this.currentAccount.address,
                });
